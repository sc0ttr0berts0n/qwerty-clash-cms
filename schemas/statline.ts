...
export default defineType({
  ...
  fields: [
    ...
    defineField({
      ...
      validation: (Rule) =>
        Rule.regex(/^([0-5]?[0-9]):([0-5]?[0-9])$/, 'Time must be in mm:ss format'),
    }),
    defineField({
      name: 'kos',
      type: 'number',
      title: 'KOs',
      fieldset: 'post_match',
      validation: (Rule) => Rule.positive().max(6).integer(),
    }),
    defineField({
      name: 'falls',
      type: 'number',
      title: 'Falls',
      fieldset: 'post_match',
      validation: (Rule) => Rule.positive().max(5).integer(),
    }),
    defineField({
      name: 'self_destructs',
      type: 'number',
